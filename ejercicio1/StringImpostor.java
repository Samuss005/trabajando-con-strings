...
public class StringImpostor {
    ...
    // Método sencillo de IndexOf: busca el carácter c desde la posición n
    public static int Indice(String s, char c, int n) {
        for (int i = n; i < s.length(); i++) {
            if (s.charAt(i) == c) {
                return i; // Si encontramos el carácter devolvemos su posición
            }
        }
    ...
    // Método sencillo de LastIndexOf: busca el carácter c desde la posición m hacia atrás
    public static int UltimoIndice(String s, char c, int m) {
        if (s == null || s.isEmpty()) {
            return -1; // Si la String está vacía o es null devolvemos -1
        }
        for (int i = m; i >= 0; i--) {
            if (s.charAt(i) == c) {
                return i; // Si encontramos el carácter devolvemos su posición
    ...
    // Método sencillo de trim: elimina espacios al principio y al final
    public static String SinSangria(String s){

        int inicio = 0; // Inicio de la String
        int fin = s.length() - 1; // Final de la String
        ...
        // Construimos la nueva String sin espacios al inicio ni al final
        for (int i = inicio; i <= fin; i++) {
            resultado = resultado + s.charAt(i);
        }

        return resultado; // Devolvemos la String limpia
    }
    ...
    // Método sencillo de substring: devuelve los caracteres desde a hasta b-1
    public static String CortarString(String s, int a, int b){

        String resultado = "";

        // Recorrer desde la posición a hasta b-1
        for (int i = a; i < b; i++) {
            resultado = resultado + s.charAt(i);
        }

        return resultado; // Devolvemos la nueva String
    }
